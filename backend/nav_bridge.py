...
class NavBridge:
    ...
    def init_ros(self):
        """Initialize rclpy and create a persistent node and publisher."""
        if not rclpy.ok():
            rclpy.init()
    ...
    def publish_goal(self, dock: dict, rack_id: str):
        try:
            ...
            # Using a simplified approach for the bridge: just store the handle and update status
            # In a more complex bridge, we'd handle callbacks for feedback/result
    ...
    def stop(self):
        ...
        # Also publish current pose or just let Nav2 handle the cancel
        # For immediate stop, we ensure status is IDLE
        self.set_status("IDLE")
        with _lock:
            self._goal_handle = None
...
def navigate_to_dock(rack_id: str, dock_point: dict) -> dict:
    ...
    # Action calls are async, so we don't necessarily need a separate thread for the *call*
    # but we do it to avoid blocking the main API thread during server wait
